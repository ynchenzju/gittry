...
function! Powerline#Functions#GetMode() " {{{
	...
	elseif mode ==# 'V'
		let mode = get(g:, "Powerline_mode_V", "VINE")
	elseif mode ==# ''
		let mode = get(g:, "Powerline_mode_cv", "VBLOCK")
	elseif mode ==# 's'
		let mode = get(g:, "Powerline_mode_s", "SELECT")
	elseif mode ==# 'S'
		let mode = get(g:, "Powerline_mode_S", "SLINE")
	elseif mode ==# ''
		let mode = get(g:, "Powerline_mode_cs", "SBLOCK")
	elseif mode =~# '\vi'
...
function! Powerline#Functions#GetCharCode() " {{{
	...
	silent! ascii
	redir END
if match(ascii, 'NUL') != -1 return 'NUL' endif " Zero pad hex values let nrformat = '0x%02x' let encoding = (&fenc == '' ? &enc : &fenc) if encoding == 'utf-8' " Zero pad with 4 zeroes in unicode files
		let nrformat = '0x%04x'
	endif
